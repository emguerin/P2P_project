...
public class WelcomeServer {
	...
	public static void main(String[] args) {
		...
		ServerSocket serverSock = new ServerSocket(portNumber);) {
			...
			while (true) {
				try (
				// Tout ce qui est dans ce bloc sera fermé automatiquement à la
				// fin du try
				Socket s = serverSock.accept();
					...
					// Après avoir accepté une connexion on lit immédiatement un
					// message
					String inputLine;
					if ((inputLine = in.readLine()) != null) {
						...
						if (words.length < 1) {
							...
							case "rt?":
								// Une demande de table de routage, on la
								// transmet
								if (words.length != 2) {
									out.println("wrq");
								} else {
							...
							case "li?":
								// Une demande de la liste des présents, on la
								// transmet
								for (Map.Entry<Integer, String> e : inNetwork
										.entrySet())
					...
					System.err
							.println("Erreur pendant une communication (WelcomeServer)");
